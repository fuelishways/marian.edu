...
if (__webpack_require__(8)) {
  ...
  var $typed = __webpack_require__(40);
...
var _givingtrail = __webpack_require__(195);

var _givingtrail2 = _interopRequireDefault(_givingtrail);

var _donor = __webpack_require__(196);
...
new _givingtrail2.default();
new _donor2.default();
new _modal2.default();
new _progressBar2.default();
...
var $export = __webpack_require__(0);
$export($export.G + $export.W + $export.F * !__webpack_require__(40).ABV, {
...
var Header = {
    ...
    init: function init() {
        this.header = document.querySelector('.' + this.selector);

        if (!this.header) return;

        var scroll = this.debounce(this.handleScroll.bind(this), 10, true);

        window.addEventListener('scroll', scroll);
    },
    handleScroll: function handleScroll() {
        var scrollPos = window.scrollY || window.pageYOffset;
        var heroText = document.querySelector('.hero .l-contain');

        if (window.innerWidth < 992) return;

        if (scrollPos >= this.getOffsetTop(heroText) - 100) {
            this.morph();
        } else {
            this.unmorph();
        }
    ...
    morph: function morph() {
        this.header.classList.add(this.selector + '--scrolled');
    },
    unmorph: function unmorph() {
        this.header.classList.remove(this.selector + '--scrolled');
    },
...
var Slider = function () {
    function Slider() {
        ...
        this.hook = '.slider';
        this.slider = document.querySelector(this.hook);
        if (!this.slider) return;

        this.slideHolder = this.slider.querySelector('.slides');
        this.index = 0;
        this.slides = Array.from(document.querySelectorAll('.slide'));

        this.slideHolder.classList.add('animate');
        this.slideHolder.style.width = this.slides.length * 100 + 'vw';
        this.slideWidth = this.slideHolder.offsetWidth;
    ...
    /**
     * Add eventListeners to all slider buttons.
     */
    ...
    _createClass(Slider, [{
        ...
        value: function bindClicks() {
            ...
            var prev = document.querySelector('.slider-control--left');
            var next = document.querySelector('.slider-control--right');
            var nextButtons = Array.from(document.querySelectorAll('.js-next'));

            nextButtons.forEach(function (item) {
                item.addEventListener('click', _this.nextSlide.bind(_this));
            });
        ...
        /**
         * Bind touch events to the slider.
         */
        ...
        /**
         * Add eventListeners for previous and next arrow keys.
         */
        ...
        value: function setSlide() {
            ...
            this.setTranslateX(slides, '-' + this.index / this.slides.length * 100 + '%');

            this.after();
        }

        /**
         * Set translateX on an element with cross-browser support.
         *
         * @param {object} el The element to translate.
         * @param {string} value The value for translateX()
         */

    }, {
        key: 'setTranslateX',
        value: function setTranslateX(el, value) {
            var transfromString = '';
            if (value != 0) {
        ...
        /**
         * Provides a hook for after slide movement.
         */
        ...
        /**
         * Update the next slide preview text and number.
         */
        ...
        value: function updatePreview() {
            ...
            var previewNumber = this.slider.querySelector('.slide-next__number');
            var previewTitle = this.slider.querySelector('.slide-next__title');

            var nextSlide = this.slides[this.index + 1];

            if (nextSlide) {
                var nextTitle = nextSlide.querySelector('.slide__subtitle h3').textContent;
                var nextNumber = nextSlide.querySelector('.slide__number').textContent;

                previewTitle.textContent = nextTitle;
                previewNumber.textContent = nextNumber;
            } else {
        ...
        value: function updateButtonText() {
            this.slides.forEach(function (slide, index, slides) {
                var buttons = Array.from(slide.querySelectorAll('.js-next'));

                var nextSlide = slides[index + 1];
                if (!nextSlide) return;

                var text = slides[index + 1].querySelector('.slide__subtitle h3').textContent;
        ...
        value: function touchStart(e) {
            ...
            // The location of the touch start
            this.touchstartx = e.touches[0].pageX;
            // the starting translateX value
            this.startPos = this.index / this.slides.length * 100;
        }

        /**
         *
         * @param {object} e The event logic.
         */
        ...
        value: function touchMove(e) {
            ...
            // remove animation on swipe (we handle this programatically)
            this.slideHolder.classList.remove('animate');

            // calculate distance of swipe
            var distance = this.touchstartx - this.touchmoveX;
            this.forward = distance > 0 ? true : false;

            if (!this.shouldSwipe()) return;

            var sliderWidth = this.slides.length * 100;
            this.moveX = this.startPos + distance / sliderWidth * 5;

            // animate the swipe
            this.setTranslateX(this.slideHolder, '-' + this.moveX + '%');
        }
    }, {
        key: 'touchEnd',
        value: function touchEnd() {
            // if there is no move event, exit.
            if (!this.touchmoveX) return;

            // find the movement percentage
            var absMove = Math.abs(this.touchstartx - this.touchmoveX) / window.innerWidth;

            if (absMove > .15) {
                if (this.forward && this.index < this.slides.length) {
                    this.nextSlide();
                } else if (!this.forward && this.index > 0) {
                    this.prevSlide();
                }
            } else {
                this.setTranslateX(this.slideHolder, '-' + this.startPos + '%');
            }
        ...
        /**
         * Determine if the slider should move on swipe.
         */

    }, {
        key: 'shouldSwipe',
        value: function shouldSwipe() {
            // if we are moving forward and we are on the last slide
            if (this.index + 1 == this.slides.length && this.forward) return false;

            // we are moving backwards and on the first slide
            if (this.index == 0 && !this.forward) return false;
...
var GivingTrail = function () {
    function GivingTrail() {
        var _this = this;

        _classCallCheck(this, GivingTrail);

        this.titles = Array.from(document.querySelectorAll('.giving-trail__button'));
        if (!this.titles) return;

        this.items = Array.from(document.querySelectorAll('.giving-trail__item'));

        this.titles.forEach(function (title) {
            title.addEventListener('click', _this.handleClick.bind(_this));
        });
    }

    _createClass(GivingTrail, [{
        key: 'handleClick',
        value: function handleClick(e) {
            this.lastClicked = e.currentTarget;
            var trailItem = e.currentTarget.parentNode;
            this.handleClose();

            this.closeAll(trailItem);

            if (!this.isActive(trailItem)) {
                trailItem.classList.add('giving-trail__item--active');
            } else {
                trailItem.classList.remove('giving-trail__item--active');
            }
        }
    }, {
        key: 'handleClose',
        value: function handleClose() {
            ...
            window.addEventListener('keyup', function handleClose(e) {
                if (e.keyCode == 27) {
                    self.closeAll();
            ...
            window.addEventListener('click', function handleClickClose(e) {
                if (e.target != self.lastClicked) {
                    self.closeAll();
        ...
        key: 'closeAll',
        value: function closeAll(except) {
            this.items.forEach(function (item) {
                if (item != except) {
                    item.classList.remove('giving-trail__item--active');
                }
            });
        }
    }, {
        ...
        value: function isActive(element) {
            return element.classList.contains('giving-trail__item--active');
        }
    }]);
...
var Donor = function Donor() {
    ...
    this.donors = Array.from(document.querySelectorAll('.donor'));

    if (!this.donors) return;

    var chosen = this.donors[Math.floor(Math.random() * this.donors.length - 1) + 1];

    this.donors.forEach(function (donor) {
        if (donor !== chosen) {
            donor.classList.add('donor--hidden');
        }
    });
};
...
var _youtubeIframe = __webpack_require__(198);
...
var Modal = function () {
    function Modal() {
        ...
        this.openButtons = Array.from(document.querySelectorAll('.js-video'));
        this.player;
        this.videoId = document.getElementById('player').getAttribute('data-videoId');
        ...
        this.openButtons.forEach(function (button) {
            button.addEventListener('click', _this.openModal.bind(self));
        });
    }
    ...
    _createClass(Modal, [{
        ...
        value: function openModal() {
            ...
            this.getFirstFocusable(this.modal).focus();

            // Listen for things that should close the dialog
            Array.from(this.modal.querySelectorAll('.js-modal__close')).forEach(function (item) {
                return item.addEventListener('click', _this2.closeModal.bind(_this2));
            });
            ...
            document.addEventListener('keydown', function (e) {
                _this2.trapTabKey(e, _this2.modal);
            });
        }
    }, {
        key: 'loadVideo',
        ...
        value: function closeModal() {
            ...
            Array.from(this.modal.querySelectorAll('.js-modal__close')).forEach(function (item) {
                return item.removeEventListener('click', _this3.closeModal.bind(_this3));
            });
            ...
            document.removeEventListener('keydown', function (e) {
                _this3.trapTabKey(e, _this3.modal);
            });

            this.player.stopVideo();

            // De-inertify all the siblings.
            this.removeInert(this.modal);
        ...
        key: 'toggleInert',
        value: function toggleInert(element) {
        ...
        value: function getFirstFocusable() {
            ...
            var focusable = this.getFocusable(context);
            return focusable[0];
        }
    }, {
        key: 'getFocusable',
        value: function getFocusable() {
            ...
            var focusable = Array.from(context.querySelectorAll('button, [href],  select, textarea, input:not([type="hidden"]), [tabindex]:not([tabindex="-1"])'));
            return focusable;
        }
    }, {
        key: 'removeInert',
        value: function removeInert(element) {
            Array.from(document.body.children).forEach(function (child) {
                if (child !== element && child.tagName !== 'LINK' && child.tagName !== 'SCRIPT') {
                    child.inert = false;
                }
        ...
        key: 'checkCloseModal',
        value: function checkCloseModal(e) {
            if (e.keyCode === 27) {
                this.closeModal();
            }
        }
        ...
        key: 'trapTabKey',
        value: function trapTabKey(e, context) {
            if (e.key !== 'Tab') return;

            var focusableItems = this.getFocusable(context);
...
var ProgressBar = function () {
    function ProgressBar() {
        ...
        this.progress_bar = document.querySelector('.progress__bar');
        this.progress_fill = this.progress_bar.querySelector('.progress__bar-fill');
        this.progress_amount = this.progress_bar.querySelector('.progress__bar-amount');

        this.position_checker = this.throttle(this.checkPosition.bind(this), 500);

        //run both events through same throttle
        window.addEventListener('scroll', this.position_checker);
        window.addEventListener('resize', this.position_checker);

        //do an intial position check on load
        this.checkPosition();
    }

    /**
     * Check where the progress bar is relative to the bottom of the window
     */
    ...
    _createClass(ProgressBar, [{
        ...
        value: function checkPosition() {
            var bar_top = this.progress_bar.getBoundingClientRect();

            //capture the first time the progress bar comes into view
            if (window.innerHeight > bar_top.top) {
                this.activateProgressBar();
            }
        }

        /**
         * Animate the progress bar by assigning width to the fill
         * Remove any listeners that are hitting the check position function
         */

    }, {
        key: 'activateProgressBar',
        value: function activateProgressBar() {
            //determine a percentage width using the amount figures
            var percentage = Math.ceil((this.progress_amount.dataset.amount ? this.progress_amount.dataset.amount : 75000) / 150000000 * 100);

            this.progress_fill.style.width = percentage + '%';

            window.removeEventListener('resize', this.position_checker);
            window.removeEventListener('scroll', this.position_checker);
        }

        /**
         * Fire function func once per every interval
         * @param func
         * @param interval
         * @returns {Function}
         */
        ...
        key: 'throttle',
        value: function throttle(func, interval) {
            var timeout;
            ...
            return function () {
                ...
                //if there is a pending timeout, do nothing
                if (timeout) {
                    return;
                }
